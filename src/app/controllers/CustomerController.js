...
class CustomerController {
        ...
        Promise.all([
          ...
          for (var customer of customers) {
            ...
            for (var reception of receptions) {
              if (
                reception.of_customer._id.toString() == customer._id.toString()
              ) {
                debt += reception.debt;
              }
            }
  ...
  showhistory(req, res, next) {
    Position.findOne({ _id: res.locals.employee.position })
      ...
      .then((position) => {
        Customer.find({})
          .populate("of_reception", ["name", "debt"])
          .then((customers) => {
            for (const customer of customers) {
              var totaldebt = 0;
  ...
  customerHistoryDetail(req, res, next) {
    Position.findOne({ _id: res.locals.employee.position })
      ...
      .then((position) => {
        Customer.find({ _id: req.params.id })
          .populate({ path: "of_reception", populate: { path: "brand" } })
          .then((customers) => {
            var listreception = [];
            for (const customer of customers) {
              for (const reception of customer.of_reception) {
