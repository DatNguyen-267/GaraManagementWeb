    ...
    changePassword(req, res, next) {
        Account.findOne({ of_employee: res.locals.employee._id }).then((account) => {
            if (passwordHash.verify(req.body.old_password, account.password)) {
                Account.updateOne({ of_employee: res.locals.employee._id }, { password: passwordHash.generate(req.body.new_password)}).then(() => {
                    res.redirect('/' + res.locals.employee._id + '/setting?_success=true')
                })
            } else {
