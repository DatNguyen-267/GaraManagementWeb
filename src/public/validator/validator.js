...
Validator.isRequire = function (selector, errorMessage) {
  return {
    ...
    test: function (value) {
      return value.trim()
        ? undefined
        : errorMessage || "Vui lòng không bỏ trống";
    },
  };
};
Validator.isEmail = function (selector) {
  return {
    ...
    test: function (value) {
      ...
      return regex.test(value) ? undefined : "Địa chỉ email không hợp lệ";
    },
  };
};
...
Validator.isRequireSelectBox = function (
  selector,
  textException,
  errorMessage
) {
  return {
    selector: selector,
    test: function (value) {
      if (value == textException)
        return errorMessage || "Vui lòng không bỏ trống";
...
Validator.notGreaterThan = function (selector, selectorLimited, errorMessage) {
  return {
    ...
    test: function (value) {
      ...
      if (Number.parseInt(value) > Number.parseInt(limitedValue))
        return errorMessage;
...
Validator.checkReEnter = function (selector, input1, errorMessage) {
  return {
    selector: selector,
    test: function (value) {
      if (value == document.querySelector(input1).value) return undefined;
